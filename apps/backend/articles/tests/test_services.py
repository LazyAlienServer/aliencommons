...
class ArticleServiceTests(BaseTestCase):
    ...
    def test_submit_creates_pending_snapshot_and_event(self):
        ...
        result = submit(article_id=article.id, actor=self.author)

        article.refresh_from_db()
        snapshot = ArticleSnapshot.objects.get(article=article)
        event = ArticleEvent.objects.get(article=article)

        self.assertEqual(article.status, Article.ArticleStatus.PENDING)
        self.assertEqual(snapshot.title, article.source.title)
        self.assertEqual(snapshot.markdown, article.source.markdown)
        self.assertEqual(snapshot.source_version, article.source.version)
        self.assertEqual(
    ...
    def test_save_draft_updates_content_version_and_last_saved_at(self):
        ...
        self.assertEqual(article.source.title, "New title")
        self.assertEqual(article.source.markdown, "# New title\n\nNew")
    ...
    def test_save_draft_moves_unarticle_publication_back_to_draft(self):
        article = create_article(
            author=self.author,
            status=Article.ArticleStatus.UNPUBLISHED,
        )
        ...
        article.refresh_from_db()
        self.assertEqual(article.status, Article.ArticleStatus.DRAFT)
        self.assertEqual(article.source.title, "A new draft")
        self.assertEqual(article.source.markdown, "# A new draft\n\nBody")
        self.assertEqual(article.source.version, 2)
    ...
    def test_save_draft_rejects_article_publication(self):
        article = create_article(
            author=self.author,
    ...
    def test_submit_rejects_unchanged_markdown_after_previous_snapshot(self):
        ...
        with self.assertRaises(ServiceError) as exc:
            submit(article_id=article.id, actor=self.author)

        self.assert_service_error(exc, code="no_change_error")
        self.assertEqual(ArticleSnapshot.objects.filter(article=article).count(), 1)
    ...
    def test_submit_rejects_invalid_article_markdown(self):
        article = create_article(author=self.author, markdown="Body without title")
    ...
    def test_withdraw_restores_draft_and_marks_snapshot_withdrawn(self):
        ...
        result = withdraw(article_id=article.id, actor=self.author)
    ...
    def test_approve_creates_article_publication_and_marks_snapshot_approved(self, render_mock):
        article = create_article(
            author=self.author,
            status=Article.ArticleStatus.PENDING,
        )
        ...
        result = approve(article_id=article.id, actor=self.moderator)

        article.refresh_from_db()
        snapshot.refresh_from_db()
        published = ArticlePublication.objects.get(article=article)
        event = ArticleEvent.objects.get(article=article)
    ...
    def test_reject_restores_draft_and_marks_snapshot_rejected(self):
        ...
        reject(article_id=article.id, actor=self.moderator)
    ...
    def test_unpublish_marks_article_unpublished(self):
        ...
        self.assertEqual(article.status, Article.ArticleStatus.UNPUBLISHED)
        self.assertIsNotNone(article.last_moderation_at)
        self.assertEqual(
            snapshot.moderation_status,
            ArticleSnapshot.SnapshotStatus.APPROVED,
    ...
    def test_soft_delete_marks_article_deleted_and_removes_article_publication(self):
        ...
        snapshot = create_article_snapshot(article)
        create_article_publication(article, title=snapshot.title, html=snapshot.markdown)

        result = soft_delete(article_id=article.id, actor=self.author)
    ...
    def test_soft_delete_rejects_pending_article(self):
        ...
        with self.assertRaises(ServiceError) as exc:
            soft_delete(article_id=article.id, actor=self.author)
